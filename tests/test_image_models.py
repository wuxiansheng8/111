...
class ImageModelTests(unittest.TestCase):
    def test_studio_image_model_matrix(self):
        ...
        self.assertEqual(len(nano_ids), 45)
        self.assertEqual(len(gpt_ids), 33)
        for resolution in ("1k", "2k", "4k"):
            self.assertIn(f"firefly-nano-banana2-{resolution}-auto", MODEL_CATALOG)
            ...
            self.assertIn(f"firefly-gpt-image-{resolution}-auto", MODEL_CATALOG)
            self.assertIn(f"firefly-gpt-image-{resolution}-16x9", MODEL_CATALOG)

    def test_nano_banana_auto_ratio_keeps_resolution(self):
        ...
        self.assertEqual(payload["size"], {"width": 2048, "height": 2048})
        self.assertEqual(payload["modelSpecificPayload"]["aspectRatio"], "auto")

    def test_gpt_image_auto_ratio_uses_upstream_sentinel(self):
        ...
        self.assertEqual(payload["size"], {"width": -1000, "height": -1000})
        self.assertEqual(payload["modelSpecificPayload"], {"size": "auto"})
        self.assertEqual(
