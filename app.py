...
def _resolve_video_options(data: dict) -> tuple[bool, str, str]:
    ...
    if reference_mode not in {"frame", "image", "media"}:
    ...
    while True:
        ...
        try:
            ...
            if auth_status == "invalid":
                ...
                refresh_message = f"{refresh_message}; upstream: {exc}"
...
def _load_input_media(messages) -> list[LoadedMedia]:
    ...
    for source in media_sources:
        ...
        if media_url.startswith("data:"):
            try:
                media_bytes, mime_type = _data_url_to_bytes(media_url)
            except ValueError as exc:
                raise HTTPException(status_code=400, detail=str(exc))
        else:
            if not media_url.lower().startswith(("http://", "https://")):
                raise HTTPException(
                    status_code=400,
                    detail=f"Only http/https or data URL {media_type} inputs are supported",
                )
            resp = requests.get(media_url, timeout=60)
            if resp.status_code != 200:
                raise HTTPException(
                    status_code=400,
                    detail=f"Failed to fetch {media_type}_url: {resp.status_code}",
                )
            media_bytes = resp.content
            mime_type = (resp.headers.get("content-type") or default_mime).split(
                ";"
            )[0].strip()

        normalized_mime = str(mime_type or default_mime).lower()
        if normalized_mime == "image/jpg":
            normalized_mime = "image/jpeg"
        if normalized_mime not in allowed_mimes[media_type]:
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported {media_type} content type: {normalized_mime}",
            )
        if not media_bytes:
